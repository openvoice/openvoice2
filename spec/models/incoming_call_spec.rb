...
shared_examples "recording a voicemail" do
  ...
  it 'should ask the caller to leave a message' do
    incoming :result_iq, @incoming_call_jid # stop music
  ...
  it 'should start recording a message' do
    incoming :result_iq, @incoming_call_jid # stop music
    incoming :result_iq, @incoming_call_jid, last_command.id # say 'please leave a message'
    incoming :say_success_presence, @incoming_call_jid
  ...
  context 'when taking the message' do
    before do
      ...
      incoming :result_iq, @incoming_call_jid, last_command.id # say 'please leave a message'
      incoming :say_success_presence, @incoming_call_jid
      incoming :result_iq, @incoming_call_jid, last_command.id # recording started
    end

    it 'should not play a message if the caller hangs up during the recording' do
      incoming :recording_hangup_presence, @incoming_call_jid
      incoming :hangup_presence, @incoming_call_jid
    ...
    it 'should play a message at the end of the message recording' do
      incoming :recording_stop_presence, @incoming_call_jid # recording finished
    ...
    it 'should hangup the caller when the message is complete' do
      incoming :recording_stop_presence, @incoming_call_jid # recording finished
      incoming :result_iq, @incoming_call_jid # 'message complete' say
      incoming :say_success_presence, @incoming_call_jid
...
describe IncomingCall do
  ...
  before do
    ...
    @incoming_call_jid = "#{@incoming_call_id}@#{@domain}"
  ...
  context 'when incoming call is for an unknown user' do
    before do
      incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:unknown-user@example.com>"
    end
  ...
  context 'when incoming call is for a known openvoice number' do
    ...
    it 'should answer' do
      incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:#{@openvoice_number}@example.com>"
      last_command.should == Connfu::Commands::Answer.new(:call_jid => @incoming_call_jid, :client_jid => @client_jid)
    end
  ...
  context 'when incoming call is for a known openvoice address' do
    it 'should answer' do
      incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:known-user@example.com>", :from => "tel:+44790012345"
      last_command.should == Connfu::Commands::Answer.new(:call_jid => @incoming_call_jid, :client_jid => @client_jid)
    end
    ...
    context 'when openvoice user has not recorded a greeting' do
      before do
        incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:known-user@example.com>", :from => "tel:+44790012345"
        incoming :result_iq, @incoming_call_jid, last_command.id
      end

      it 'should then say "please wait while we transfer your call" to the caller' do
        last_command.should == Connfu::Commands::Say.new(:text => "please wait while we transfer your call", :call_jid => @incoming_call_jid, :client_jid => @client_jid)
      end
    end
    ...
    it 'should then play music to the caller' do
      ...
      last_command.should == Connfu::Commands::Say.new(:text => "http://www.phono.com/audio/troporocks.mp3", :call_jid => @incoming_call_jid, :client_jid => @client_jid)
    end

    context 'with one endpoint' do
      before do
        @openvoice_endpoint_call_id = "openvoice-endpoint-call-id"
        @openvoice_endpoint_call_jid = "openvoice-endpoint-call-id@#{@domain}"

        @endpoint_one = Factory(:endpoint, :account => @account)

        incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:known-user@example.com>"
        incoming :result_iq, @incoming_call_jid, last_command.id
        incoming :result_iq, @incoming_call_jid, last_command.id
        incoming :say_success_presence, @incoming_call_jid
        incoming :result_iq, @incoming_call_jid, last_command.id
      ...
      describe "once the dial has happened" do
        before do
          incoming :dial_result_iq, @openvoice_endpoint_call_id, last_command.id
        end
        ...
        it 'should hangup the caller if the openvoice endpoint rejects the call' do
          incoming :reject_presence, @openvoice_endpoint_call_jid

          last_command.should == Connfu::Commands::Hangup.new(:call_jid => "#{@incoming_call_id}@#{@domain}", :client_jid => @client_jid)
        end

        it 'should wait for one of the parties to hang up' do
          incoming :ringing_presence, @openvoice_endpoint_call_jid
        ...
        describe "and the call openvoice endpoint has answered" do
          before do
            incoming :answered_presence, @openvoice_endpoint_call_jid # openvoice endpoint answers
          end
          ...
          it 'should hangup the caller when the openvoice endpoint hangs up' do
            incoming :hangup_presence, @openvoice_endpoint_call_jid # openvoice endpoint hangs up
            incoming :result_iq, @incoming_call_jid, last_command.id # server responds to expected Hangup command for the caller
            incoming :hangup_presence, @incoming_call_jid # caller hangs up

            last_command.should == Connfu::Commands::Hangup.new(:call_jid => @incoming_call_jid, :client_jid => @client_jid)
            Connfu.should be_finished
          end

          it 'should hangup the openvoice endpoint when the caller hangs up' do
            incoming :hangup_presence, @incoming_call_jid
            incoming :result_iq, @openvoice_endpoint_call_jid, last_command.id # server responds to expected Hangup command for the openvoice user
            incoming :hangup_presence, @openvoice_endpoint_call_jid

            last_command.should == Connfu::Commands::Hangup.new(:call_jid => @openvoice_endpoint_call_jid, :client_jid => @client_jid)
            Connfu.should be_finished
          end
        end
      end
    ...
    context 'with two endpoints' do
      before do
        @openvoice_endpoint_1_call_id = 'openvoice-endpoint-call-id'
        @openvoice_endpoint_1_call_jid = "openvoice-endpoint-call-id@#{@domain}"

        @openvoice_endpoint_2_call_id = "unanswered-joined-call-id"
        @openvoice_endpoint_2_call_jid = "#{@openvoice_endpoint_2_call_id}@#{@domain}"
      ...
      context 'and using the parallel dial strategy' do
        before do
          ...
          incoming :offer_presence, @incoming_call_jid, @client_jid, :to => "<sip:known-user@example.com>"
          incoming :result_iq, @incoming_call_jid, last_command.id # answer
          incoming :result_iq, @incoming_call_jid, last_command.id # say 'please wait...'
          incoming :say_success_presence, @incoming_call_jid       # end of say 'please wait...'
          incoming :result_iq, @incoming_call_jid, last_command.id # play music
        end
        ...
        it 'should immediately ring both endpoints without waiting for the music to finish' do
          ...
          incoming :result_iq, @incoming_call_jid, last_command.id # nested join
        ...
        context 'having dialed both endpoints' do
          before do
            incoming :dial_result_iq, @openvoice_endpoint_1_call_id, last_command.id
            incoming :dial_result_iq, @openvoice_endpoint_2_call_id, last_command.id
          end
          ...
          it 'should hang up the unanswered leg when the other leg is answered' do
            incoming :answered_presence, @openvoice_endpoint_1_call_jid

            last_command.should == Connfu::Commands::Hangup.new(:call_jid => @openvoice_endpoint_2_call_jid, :client_jid => @client_jid)
          end

          it 'should wait for one of the parties to hang up' do
            incoming :answered_presence, @openvoice_endpoint_1_call_jid
            incoming :result_iq, @openvoice_endpoint_2_call_jid, last_command.id
            incoming :hangup_presence, @openvoice_endpoint_2_call_jid

            Connfu.should_not be_finished
          end

          it 'should hangup the caller when both openvoice endpoints reject the call' do
            incoming :reject_presence, @openvoice_endpoint_1_call_jid
            incoming :reject_presence, @openvoice_endpoint_2_call_jid

            incoming :result_iq, @incoming_call_jid
            incoming :hangup_presence, @incoming_call_jid
          ...
          it 'should allow one to reject and another openvoice number to answer' do
            ...
            incoming :answered_presence, @openvoice_endpoint_2_call_jid
            incoming :result_iq, @openvoice_endpoint_2_call_jid, last_command.id
          ...
          it 'should hangup the caller when the openvoice endpoint hangs up' do
            ...
            incoming :hangup_presence, @openvoice_endpoint_1_call_jid
          ...
          it 'should hangup the openvoice endpoint when the caller hangs up' do
            ...
            last_command.should == Connfu::Commands::Hangup.new(:call_jid => @openvoice_endpoint_1_call_jid, :client_jid => @client_jid)
          end
        end
      end
      ...
      context 'and using the round-robin dial strategy' do
        before do
          ...
          incoming :result_iq, @incoming_call_jid, last_command.id # say 'please wait...'
          incoming :say_success_presence, @incoming_call_jid       # end of say 'please wait...'
          incoming :say_result_iq, @incoming_call_jid, "hold-music-component-id" # play music
        end
        ...
        context 'once an endpoint answers' do
          before do
            ...
            incoming :result_iq, @incoming_call_id, last_command.id # join
          end
          ...
          context 'and the call is joined' do
            before do
              incoming :joined_presence, @incoming_call_jid, "answering-endpoint-call-id" # join complete to original call
              incoming :joined_presence, "answering-endpoint-call-id@server.whatever", @incoming_call_id # join complete to new call
            end
            ...
            it 'should hangup the openvoice leg when the incoming caller hangs up' do
              incoming :hangup_presence, @incoming_call_jid # hangup from incoming leg
  ...
  context 'when incoming call is for a known openvoice user with a recorded greeting' do
    ...
    it 'should then play recording greeting to the caller' do
      ...
      last_command.should == Connfu::Commands::Say.new(:text => @account.greeting_path, :call_jid => @incoming_call_jid, :client_jid => @client_jid)
    end
  end
end